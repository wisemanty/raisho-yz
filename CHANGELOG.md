...
- 当 `原始数据/` 目录存在 `youzan_flow_metrics_*.json` 时，Word 和 Markdown 总结自动新增 `流量与浏览转化` 章节。
- 新增内容包括：
  - 小程序 / 全店 / H5 访客、浏览量、访问-支付转化率、访问-下单转化率、平均停留、人均浏览、加购人数、支付金额。
  - 微页面 UV 排行。
  - 商详页 UV 排行。
  - 商品加购排行。
- 商品 UV 和加购排行会按经营品类合并，避免同一品类多个链接在周会报告里重复分散。
- 若没有流量 JSON，报告保持原有结构，不阻塞订单分析。

## 2026-06-15 v0.11.2

### 非周度区间口径修正

- 当分析区间超过 7 天时，经营总结自动使用 `本期 / 上期 / 下一阶段`。
- Word 和 Markdown 的标题、章节名、核心看板、商品表、客户池、分销员表、行动清单和对比文案都会同步改口径。
- 周度报告仍保留 `本周 / 上周 / 下周`，避免每周一固定周会的表述被过度泛化。

## 2026-06-15 v0.11.1

### 用户分层复购口径修正

- `01用户分层表` 中的 `复购用户` 标签、复购原因、P0/P1 优先级加分现在统一使用 `购买会话数 >= 2`。
- 同一 `yz_open_id` 在同一自然日内多笔订单只算 1 次购买会话，不再因为当天拆单被标成复购。
- 这个修正与总览、分销员质量表、Word 附录里的复购率口径保持一致。

## 2026-05-25 v0.11.0

### Word 周会版改版

- `经营分析总结_<week>.docx` 和 Markdown 总结改为周会投屏大纲。
- 新大纲固定为：
  1. 本周一句话结论
  2. 核心数据看板
  3. 本周变化
  ...
  9. 下周要验证的问题
  10. 附录：数据口径
- Word 不再以商品路径为主，商品路径只保留在 Excel 追溯表里。
- 分销员销售排行和经营判断成为 Word 主体章节。
- Word 改为横版页面，更适合周会投屏。
- 分销员章节拆成 `销售排行` 和 `下周动作` 两张表，避免宽表挤压。

### 数据增强

- 周会总结会同时读取本周过滤明细和全量明细。
- 核心数据看板新增：
  - 累计成交额
  - 累计有效订单数
  - 累计成交客户数
  - 本周新客 / 老客
  - 与上一自然周期的成交额对比
- 客户经营池按可执行动作组织，不再只是用户标签展示。

### 验证

- 已安装/配置 LibreOffice 渲染链路。
- 已将 `2026-05-18 至 2026-05-24` 的 Word 总结转成 PDF 并渲染为 4 页 PNG。
- 页面检查结果：10 段大纲完整，分销员排行可读，无空白页。
